...
import { deleteUserProviderById } from 'apiSdk/user-providers';
...
function ProviderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProviderInterface>(
    ...
    () =>
      getProviderById(id, {
        relations: ['user', 'content', 'user_provider'],
      }),
  );
  ...
  const user_providerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteUserProviderById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('user_provider', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      User Providers
                    </Text>
                    <NextLink passHref href={`/user-providers/create?provider_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.user_provider?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/user-providers/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('user_provider', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/user-providers/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('user_provider', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    user_providerHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
